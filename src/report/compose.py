...
def make_sold_report(root_path, report_path):
    ...
    df['sold_price'] = df.apply(lambda x: '{:,}'.format(x.sold_price).replace(',', '.'), axis=1)
    df['sqm_price'] = df.apply(lambda x: '{:,}'.format(x.sqm_price).replace(',', '.'), axis=1)
    df['price_diff'] = df.apply(lambda x: '{:,}'.format(x.price_diff).replace(',', '.'), axis=1)


    # filtering
    cols = ['address', 'zip_code', 'rooms', 'living_area', 'build_year', 'list_price', 'sold_price', 'sqm_price', 'price_diff', 'url', 'days_since_sale']
...
def make_for_sale_report(root_path, report_path):
    ...
    # filtering
    cols = ['zip_code', 'street', 'rooms', 'living_area', 'lot_area', 'energy_class', 'build_year', 'list_price', 'url', 'days_on_market']
